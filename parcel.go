...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	res, err := s.db.Exec(`ISERT INTO parcel (client, status, address, created_at) 
...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	row := s.db.QueryRow(`SELECT client, status, address, created_at FROM parcel
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	for rows.Next() {
		parcel := Parcel{}
		err := rows.Scan(&parcel.Number, &parcel.Client, &parcel.Status,
			&parcel.Address, &parcel.CreatedAt)
		if err != nil {
			return nil, err
		}
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	_, err := s.db.Exec(`UPDATE parcel SET address = :address WHERE 
	number = :number AND status = 'registered'`,
...
func (s ParcelStore) Delete(number int) error {
	...
	AND status = 'registered'`, sql.Named("number", number))
	if err != nil {
		return err
	}
